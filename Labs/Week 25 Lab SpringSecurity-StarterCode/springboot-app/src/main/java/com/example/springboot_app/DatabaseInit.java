...
import com.example.springboot_app.Models.*;
import com.example.springboot_app.Repos.*;
...
public class DatabaseInit implements CommandLineRunner{
	...
	@Override
	 public void run(String... args) throws Exception {
		...
		Iterable<Produce> produces = Objects.requireNonNull(Arrays.<Produce>asList(apple, lettuce, potato));

		produceRepository.saveAll(produces);
		...
		Iterable<SellerProduce> sellerProduces = Objects.requireNonNull(Arrays.asList(Alice_apple, Alice_lettuce, Carol_apple, Carol_potato));	

		sellerProduceRepository.saveAll(sellerProduces);
		...
		Iterable<OrderItem> orderItems = Objects.requireNonNull(Arrays.asList(Bob_apple, Bob_lettuce));
		orderItemsRepository.saveAll(orderItems);

		SellerProduce Carol_apple_InDB = sellerProduceRepository.findById(Objects.requireNonNull(Carol_apple.getId())).get();
		Carol_apple_InDB.setQuantity(Carol_apple_InDB.getQuantity()-2);
		sellerProduceRepository.save(Carol_apple_InDB);
		SellerProduce Alice_lettuce_InDB = sellerProduceRepository.findById(Objects.requireNonNull(Alice_lettuce.getId())).get();
		...
		Iterable<OrderItem> Altti_orderItems = Objects.requireNonNull(Arrays.asList(Altti_apple,Altti_potato));
		orderItemsRepository.saveAll(Altti_orderItems);

		SellerProduce Alice_Apple_InDB = sellerProduceRepository.findById(Objects.requireNonNull(Alice_apple.getId())).get();
		Alice_Apple_InDB.setQuantity(Alice_Apple_InDB.getQuantity()-10);
		sellerProduceRepository.save(Alice_Apple_InDB);
		
		SellerProduce Carol_potato_InDB = sellerProduceRepository.findById(Objects.requireNonNull(Carol_potato.getId())).get();
